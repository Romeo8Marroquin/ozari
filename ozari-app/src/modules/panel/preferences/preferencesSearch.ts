/**
 * The four groups of the preferences screen, as URL state.
 *
 ...
 */
export type PreferenceTab = 'operation' | 'orders' | 'products' | 'documents';

/** The segmented control's option order (also the tabs' arrow-key order). */
export const PREFERENCE_TABS: readonly PreferenceTab[] = [
  'operation',
  'orders',
  'products',
  'documents',
...
export interface PreferencesSearch {
  grupo?: 'pedidos' | 'productos' | 'documentos';
}
...
/**
 * Clamp-never-reject (the products filters' stance): anything that isn't one of the non-default
 * markers — absent, misspelled, the default spelled out — parses to the default group, as an EMPTY
